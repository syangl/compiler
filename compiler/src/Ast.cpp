 ...
 * 
 * 语法树：
 *      语法树的结点封装成对象，包含类型检查和中间代码生成的函数 main.c -> 语法分析生成ast ==> ast->typecheck() ==> ast->genCode() ==> unit->machineCode()
 * 
 * 控制流的翻译：
 *      通过回填技术来完成控制流的翻译。我们为每个结点设置两个综合属性true_list 和false_list, 它们是跳转目标未确定的跳转指令的列表，
 *      回填是指当跳转的目标基本块确定时，设置list中跳转指令的跳转目标为该基本块。（具体例子可见布尔表达式AND的注释说明）
 * 
 * 中间代码生成：
 *      genCode如同手写代码那样new Instruction生成指令，Instruction有output将llvm IR写出到文件中
 * 
 * 
 * 目标代码生成：
 *      genMachineCode整体逻辑（类似于中间代码生成）：利用已经生成的中间代码基本块生成对应的目标代码基本块，二者建立map映射（用于为machineBlock前后相邻块拉双链）；对中间代码进行自顶向下的遍历，从而生成目标代码。
 * 
 *      unit->machineCode() ==> function->machineCode() ==> map(Block, machineBlock) ==> block->genMachineCode() ==> instruction->genMachineCode() ==> linearScan.allocateRegisters()
 * 
 * 
*/
...
void Node::backPatch(std::vector<Instruction*> &list, BasicBlock*bb) // list
{
    for(auto &inst:list)
    {
        if(inst->isCond()) // 有条件
            dynamic_cast<CondBrInstruction*>(inst)->setTrueBranch(bb);
        else if(inst->isUncond()) // 无条件
...
void BinaryExpr::genCode() // note：条件分支翻译的举例
{
    BasicBlock *bb = builder->getInsertBB();
    Function *func = bb->getParent();
    if (op == AND) // 比如 if (expr1 && expr2) then     else  
    {
        BasicBlock *trueBB = new BasicBlock(func);  // if the result of lhs is true, jump to the trueBB.
        // 在第一个子表达式生成中间代码的过程中，生成的跳转指令的目标基本块尚不能确定，因此会将跳转指令插入到子表达式结点的true_list 和false_list 中。
        expr1->genCode();
        // 在翻译当前布尔表达式时，已经能确定true_list 中跳转指令的目的基本块为trueBB，因此进行回填。
        backPatch(expr1->trueList(), trueBB);
        // 重设builder的插入指针为trueBB，因为expr1为真时（expr1跳转到truebb），继续判断expr2，所以在trueBB里继续插入expr2生成的指令
        builder->setInsertBB(trueBB);               // set the insert point to the trueBB so that intructions generated by expr2 will be inserted into it.
        expr2->genCode();
        // 当前仍不能确定子表达式二的true_list 的目的基本块，因此将其插入到当前结点的true_list 中
        true_list = expr2->trueList();
        // 也不能知道两个子表达式的false_list 的跳转基本块，便只能将其插入到当前结点的false_list中，让父结点回填当前结点的true_list 和false_list。
        false_list = merge(expr1->falseList(), expr2->falseList()); // 两个false的目的地是一样的，所以合并
    }
    else if(op == OR)
    {
        // Todo
    ...
    else if(op >= LESS && op <= NOTEQUAL)
    {
        ...
        new CmpInstruction(opcode, dst, src1, src2, bb); // 递归地生成，子表达式生成完了父表达式生成

        BasicBlock *temp_bb = new BasicBlock(func);
        BasicBlock *true_bb = new BasicBlock(func);
        BasicBlock *false_bb = new BasicBlock(func);
        true_list.push_back(new CondBrInstruction(true_bb, temp_bb, dst, bb)); // 父表达式是爷表达式的子表达式，也存在回填技术
...
void IfStmt::genCode()
{
    ...
    end_bb = new BasicBlock(func); // end_bb 为if 语句后续的结点生成的中间代码的插入位置。
